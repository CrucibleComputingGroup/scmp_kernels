...
class QuantAttention(nn.Module):
    ...
    def __init__(
        self, 
        attn: Attention, # Type hint: expects a timm Attention object; library for CV
        args
    ):
        super().__init__()
        self.quantize_bmm_input = args.quantize_bmm_input
    ...
    def forward(self, x):
        ...
        # ── Step 1: Quantize activation input ────────────────────── 
        x = self.input_quant(x)
        # ── Step 2: Compute QKV ───────────────────────────────────
        # Note that W has already been quantized statically
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        # ── Step 4: RE-QUANTIZE Q, K, V separately ────────────────             
        if self.quantize_bmm_input:
            q = self.q_quant(q)
            k = self.k_quant(k)
            v = self.v_quant(v)
...
class QuantMlp(nn.Module):
    ...
    def forward(self, x):
        print(f"[QuantMlp] input: shape={x.shape}, min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}")
        if self.reorder_index_fc1 is not None:
            x = torch.index_select(x, 2, self.reorder_index_fc1)
        # quantize twice
        x = self.input_quant(x)
        print(f"[QuantMlp] after input_quant: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}, has_nan={x.isnan().any().item()}, has_inf={x.isinf().any().item()}")
        x = self.fc1(x)
        print(f"[QuantMlp] after fc1: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}, has_nan={x.isnan().any().item()}, has_inf={x.isinf().any().item()}")
        x = self.act(x)
        print(f"[QuantMlp] after act: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}")
        x = self.drop1(x)
        x = self.norm(x)
        x = self.act_quant(x)
        print(f"[QuantMlp] after act_quant: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}, has_nan={x.isnan().any().item()}, has_inf={x.isinf().any().item()}")
        x = self.fc2(x)
        print(f"[QuantMlp] after fc2: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}, has_nan={x.isnan().any().item()}, has_inf={x.isinf().any().item()}")
        x = self.drop2(x)
        print(f"[QuantMlp] output: min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}")
        return x
